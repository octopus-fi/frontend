...
export function VaultCard({
  ...
  const debtValue = BigInt(Math.floor(Number(debt) / 1e9));
  const collateralValue = BigInt(Math.floor(Number(collateral)));
  ...
  const getRiskLevel = () => {
    if (
      ...
      return "high";
    if (healthStatus.level === "warning") return "medium";
    return "low";
  };
    ...
    >
      ...
      >
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Badge variant="outline">LTV: {ltv.toFixed(1)}%</Badge>
              </div>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          {/* Health Factor Gauge */}
          <HealthFactorGauge healthFactor={healthFactor} ltv={ltv} compact />
