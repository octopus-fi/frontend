...
export default function BorrowPage() {
  ...
  const filteredVaults = mockVaults.filter(vault => {
    // Search filter
    if (searchQuery && !vault.id.toLowerCase().includes(searchQuery.toLowerCase())) {
      return false;
    }
    ...
    if (filterRisk !== 'all') {
      ...
      if (filterRisk === 'medium' && (vault.health < 1.2 || vault.health >= 1.5)) return false;
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        ...
        >
          <Card className="glass border-primary/20">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-muted-foreground">AI Protected</span>
                <AlertTriangle className="h-4 w-4 text-primary" />
              </div>
              <div className="text-2xl font-bold">
                {mockVaults.filter(v => v.aiManaged).length}/{mockVaults.length}
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                Vaults with AI
              </p>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="glass border-primary/20">
          ...
          <CardContent className="space-y-3 text-sm">
            <div className="flex gap-3">
              ...
              <div>
                <div className="font-medium mb-1">Keep Earning Staking Rewards</div>
                <p className="text-muted-foreground">
                  Your octSUI continues to earn ~7% APY even while used as collateral
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <div>
                <div className="font-medium mb-1">Zero Interest Borrowing</div>
                <p className="text-muted-foreground">
                  Pay 0% interest on borrowed octUSD - only a 0.5% one-time minting fee
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  Enable AI to monitor and auto-rebalance your vault 24/7
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="glass border-primary/20 bg-primary/5">
          ...
          <CardContent className="space-y-3 text-sm">
            <p>
              <strong>Leverage Your Yield:</strong> Borrow octUSD against your octSUI, swap it for more SUI, stake it, and repeat! This creates a leverage loop that can boost your APY to 15-20%.
            </p>
            <p className="text-muted-foreground">
              Just make sure to enable AI auto-rebalance to keep your vault safe during market volatility.
            </p>
