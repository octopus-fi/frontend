...
export default function VaultDetailPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* Health Warning */}
      {(health.status === 'warning' || health.status === 'danger' || health.status === 'liquidatable') && (
        <motion.div
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => setActiveTab('borrow')}>
                      Add Collateral
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => setActiveTab('repay')}>
                      Repay Debt
                    </Button>
                  </div>
                </div>
        ...
        >
          <Card className="glass border-primary/20">
            <CardContent className="pt-6">
              ...
              <div className="text-2xl font-bold">{formatAmount(vault.collateral)}</div>
        ...
        >
          <Card className="glass border-primary/20">
            <CardContent className="pt-6">
              ...
              <div className="text-2xl font-bold">{formatAmount(vault.debt)}</div>
              <p className="text-xs text-muted-foreground mt-1">
        ...
        >
          <Card className="glass border-primary/20">
            <CardContent className="pt-6">
              ...
              <div className="text-2xl font-bold">{health.ltv.toFixed(1)}%</div>
              <Badge 
                variant={
                  health.status === 'safe' ? 'success' :
          ...
          <Card className="glass border-primary/20">
            <CardContent className="pt-6">
              ...
              <div className="text-2xl font-bold">
                {health.healthFactor === Infinity ? '∞' : health.healthFactor.toFixed(2)}×
              </div>
              <p className="text-xs text-muted-foreground mt-1">
        ...
        <Card className="glass">
          ...
          <CardContent className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">{formatCurrency(health.availableBorrow)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Max Borrowable</span>
              <span className="font-semibold">{formatCurrency(health.maxBorrow)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Safety Reserve</span>
              <span className="font-semibold">{formatAmount(vault.rewardReserve)} octSUI</span>
            </div>
            <div className="flex justify-between">
