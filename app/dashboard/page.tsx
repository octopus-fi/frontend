    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card className="glass">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/dashboard/borrow/${vaultId}`} className="gap-2">
                    View Details
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-muted-foreground">
                        <span>Borrowed: {formatCurrency(debtValue)}</span>
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-white/5">
                      <div>
                        ...
                        <div className="font-semibold">{formatCurrency(availableBorrow)}</div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground mb-1">Safety Reserve</div>
                        <div className="font-semibold">{formatAmount(rewardReserve)} octSUI</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 rounded-lg bg-background/50 border border-white/10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">Mock SUI Token</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-mono font-semibold">{formatAmount(mocksuiBalance)}</div>
                    <div className="text-xs text-muted-foreground">
                      ~{formatCurrency((Number(mocksuiBalance) / 1e9) * octsuiPrice)}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-background/50 border border-white/10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">Liquid Staking Token</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-mono font-semibold">{formatAmount(octsuiBalance)}</div>
                    <div className="text-xs text-muted-foreground">
                      ~{formatCurrency((Number(octsuiBalance) / 1e9) * octsuiPrice)}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-background/50 border border-white/10">
                  ...
                  <div className="text-right">
                    <div className="font-mono font-semibold">{formatAmount(octusdBalance)}</div>
                    <div className="text-xs text-muted-foreground">
        ...
        <div className="space-y-6">
          {/* Health Warning */}
          {(healthStatus === 'warning' || healthStatus === 'danger' || healthStatus === 'liquidatable') && (
            <Card className="glass border-amber-500/50 bg-amber-500/5">
              <CardHeader>
                <div className="flex items-center gap-2">
                  <AlertCircle className="h-5 w-5 text-amber-500" />
                  <CardTitle className="text-amber-500">Health Warning</CardTitle>
                </div>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-muted-foreground mb-4">
                  Your vault health is {healthStatus}. Consider adding collateral or repaying debt.
                </p>
                <div className="space-y-2">
          ...
          <Card className="glass">
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">octSUI Price</span>
                <span className="font-semibold">{formatCurrency(octsuiPrice)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Staking APY</span>
                <span className="font-semibold text-green-500">{formatPercent(netAPY / 100)}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Liq. Threshold</span>
